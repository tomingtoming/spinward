...
export type ColonyModuleName='structure'|'window_frame'|'canopy'|'door'|'balcony'|'shop_awning'|'planter'|'planting'
...
export function loadColonyModules(){
  return pending??=new GLTFLoader().loadAsync('/assets/buildings/colony-modules.glb').then(g=>{
    ...
    for(const name of ['structure','window_frame','canopy','door','balcony','shop_awning','planter','planting'] as const){
