...
export class BuildingInteriorLayer {
  ...
  update(azimuth: number, axial: number, altitude: number) {
    ...
    for (const entry of this.entries) for (const { part, matrix } of entry.parts) {
      ...
      if(index<=1){const design=colonyBuildingDesign(entry.interior.building,entry.interior.kind);this.meshes[index].setColorAt(counts[index],new THREE.Color('#'+design.wall));if(index===1)writeColonyFacade(this.meshes[index],counts[index],design,0,part.y-part.height/2)}
      counts[index]++
    }
