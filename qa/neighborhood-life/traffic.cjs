...
await page.goto((process.env.SPINWARD_URL || 'https://127.0.0.1:5191')+'/?debug&m=g&a=.1247253998&ax=-325.5&t=.42');await page.waitForFunction(()=>window.__spinward?.neighborhood?.asset);
await page.evaluate(()=>{
 ...
 const along=(c.azimuth-route.laneAzimuth)*3200-route.direction*60;
...
fs.writeFileSync(require('node:path').join(__dirname,'traffic-controlled.json'),JSON.stringify({errors,samples}));console.log(JSON.stringify({samples:samples.length,phases:[...new Set(samples.map(s=>s.phase))],nearby:Math.max(...samples.map(s=>s.cars.length)),stopped:samples.filter(s=>s.cars.some(c=>c.speed<.1)).length,unsafe:samples.filter(s=>s.phase==='crossing'&&Math.abs(s.across)<.8&&s.cars.some(c=>Math.abs(c.tangent)<3)).length,errors}));}finally{await browser.close()}})();
