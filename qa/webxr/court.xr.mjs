...
// The existing certified court only exists in the desktop city budget.
// This tests stereo scenery; seating is intentionally a flat-mode interaction.
...
const a = .01939436514508575 + (-15.877007576480302 / 2 + 1.2) / 3200
const ax = -17412.754664883592 - 6.4
const at = [a + 3 / 3200, ax + 5, 3], target = [a, ax, .65]
...
position.add(new Vector3(0, -1.6, 0).applyQuaternion(rotation))

async function roomProbe(page) {
  return page.evaluate(() => {
    const layer = window.__spinwardCity.oldTownCourt
    const mesh = layer.group.getObjectByName('court-chair')
    if (!mesh?.visible || mesh.count !== 2) throw Error('Expected both nearby courtyard chairs')
    const matrix = mesh.matrix.clone(); mesh.getMatrixAt(0, matrix)
    const camera = window.__spinwardScene.getObjectsByProperty('isPerspectiveCamera', true)[0]
    const centre = camera.position.clone().set(0, .44, 0).applyMatrix4(matrix)
    return { matrix: matrix.elements, projected: centre.clone().project(camera).toArray(),
      stats: layer.group.userData, colliders: layer.plan.colliders.length, mode: window.__spinward.mode }
  })
}

test('courtyard furniture remains supported in stereo through head roll', async ({ page, xr }, info) => {
  ...
  await page.goto(`/?debug&lock=0&metrics=off&tier=desktop&dpr=1&t=.42&m=f&rpm=0&p=${position.toArray()}&q=${rotation.toArray()}`)
  await page.waitForSelector('#splash', { state: 'detached' })
  await page.waitForFunction(() => window.__spinwardCity?.oldTownCourt?.group.userData.furnitureReady)
  ...
  for (const degrees of [0, 25, -25]) {
    ...
    const path = info.outputPath(`court-roll-${degrees}.png`)
    ...
    await info.attach(`court-roll-${degrees}`, { path, contentType: 'image/png' })
  ...
  await fs.writeFile(info.outputPath('court-evidence.json'), JSON.stringify({ ...evidence, diagnostics, gpu, errors }, null, 2))
})
