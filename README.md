...
6. **雨を降らせる** — 画面下の Rain ボタン(または URL に `?rain`)。雨は回転に置いていかれて斜めに降り、傾きは高度で変わる。雲は重力の弱い軸付近に張りつくので、Axis へ向かって昇っていくと雨の上に出る。カフェや住居の屋根の下では雨を遮り、中庭の吹き抜けや入口の外では降り続ける。
...
- **全街区の建物を Blender 製へ統一**: 304棟の個別モデルと、残りの敷地を覆う共通部材キットを使用します。住宅・段状ビル・中庭型・工業棟は実寸の構造レシピで組み立て、窓は約2.8m×3.2mの間隔で繰り返します。旧建物パックは読み込みません。共通部材のソースは `assets/blender/colony-modules.blend`、生成スクリプトは `assets/blender/build_colony_modules.py`。
...
- **地区ごとの建築表現**: 港側の旧市街は暖色、業務棟は寒色、住宅は淡い塗装、工業棟は抑えた灰緑を使用。段状の上階、中庭、屋上の後退を遠景でも保持します。入れるカフェ・ロビー・住居は既存の開口と家具を維持し、一般室内の構造部材と上階外壁も共通キットへ統一します。
