...
- **近景建物は Blender 製アーキタイプ**:住宅・複合スラブ・タワーの3系統を GLB から一度だけ読み込み、LOD0(バルコニー・庇・縦フィン・段状屋根)と LOD1(簡略シルエット)をアーキタイプ単位の InstancedMesh で配置します。GLB が読めない場合は手続き生成建物へ自動フォールバックします。編集ソースは `assets/blender/spinward-buildings.blend`。
