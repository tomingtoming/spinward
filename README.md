...
- VR: 立方体へレイを向けてトリガーを引くと掴めます。
- VR: 何もない空間でトリガーを引くと手元に球を生成し、そのまま掴みます。
- VR: 球や立方体のトリガーを離すと放します。球はその時のコントローラ速度から投げ速度を決めます。
- VR: グリップを押すと立方体を手前へ戻します。
- PC: `Space` またはキャンバスをクリックで球を前方へ投げます。
- GUI: `radius`, `rpm`, `throw scale`, `surface g` を右上で確認・調整できます。
