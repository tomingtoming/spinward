...
- PC: 内壁にいる間は `WASD` で歩行します。`free-fly` 中は KSP 風の視線基準 6DOF ジェットパックで、`WASD` で前後左右、`Shift` で上昇、`Ctrl` で下降、`Q/E` でロール(バンク)します(止まるには逆方向に噴射)。`Space` はジャンプ専用で、`grounded` に戻ると視点は自動で水平へ戻ります。
