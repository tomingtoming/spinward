...
const placements=p.buildings.filter(b=>b.front&&Math.abs(b.azimuth-.05)*3200<1800&&Math.abs(b.axial)<1800&&!ins.has(b)&&!c.blocks.some(s=>s.building.azimuth===b.azimuth&&s.building.axial===b.axial)).flatMap((b,i)=>{const fits=c.blocks.filter(s=>(b.front!.axis==='tangent'?b.depth:b.width)>=s.building.width&&(b.front!.axis==='tangent'?b.width:b.depth)>=s.building.depth&&b.height>=s.building.height);if(!fits.length)return[];const previous=existing.find(p=>p.building.azimuth===b.azimuth&&p.building.axial===b.axial);const s=fits.find(s=>s.id===previous?.model)??fits[Math.abs(Math.round(b.azimuth*1e6+b.axial*10))%fits.length];return[{model:s.id,building:b,offsetZ:((b.front!.axis==='tangent'?b.width:b.depth)-s.building.depth)/2}]});
